<template>
  <div id="app">
    <div class="header-wrapper">
     ...
     <div class="tab border-1px" v-transform>
       <div class="tab-item" ref='targetUnderline'>
...
import {urlParse} from 'src/common/util/urlParse';
 function eventHandler (event, under, target) {
    if (event) {
      if (event.target.tagName === 'A') {
            let targetWidth = event.target.parentNode.offsetWidth;
            let offsetLeft = event.target.parentNode.offsetLeft;
            let underLine = this.lastElementChild;
            let selfWidth = this.lastElementChild.offsetWidth;
            underLine.style.left = offsetLeft + (targetWidth / 2 - selfWidth / 2) + 'px';
         }
    }
       if (under) {
            ...
            let underLine = under;
            let selfWidth = underLine.offsetWidth;
            underLine.style.left = offsetLeft + (targetWidth / 2 - selfWidth / 2) + 'px';
       }
    }
 // const ERRORS = 0;
  export default {
    ...
    mounted () {
      eventHandler(null, this.$refs.underLine, this.$refs.targetUnderline)
